...
export default function SmartReturnModal({ branchId, onClose, onNotify, onReturnToCart }) {
    ...
    const handleProcessReturn = async () => {
        ...
        try {
            ...
            // Build negative cart items from the returned items
            const negativeCartItems = Object.values(returnItems).map(i => ({
                id: i.productId,
                name: `[RETURN] ${i.productName}`,
                price: i.unitPrice,
                qty: -i.returnQty, // Negative quantity for returns
                discount: 0,
                taxRate: 0,
                isReturn: true, // Flag to identify return items
            ...
            // Pass returned items back to POS cart as negative values
            if (onReturnToCart) {
