...
function ServiceCard({
    ...
    return (
        <div className={`bg-white rounded-2xl p-5 border shadow-sm hover:shadow-md transition-all flex flex-col justify-between ${
            job.serviceStatus === 'ASSIGNED' ? 'border-orange-200' :
            job.serviceStatus === 'IN_PROGRESS' ? 'border-indigo-200' : 'border-slate-100'
        }`}>
            <div>
                {/* Header */}
                <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-mono text-xs font-bold text-slate-500">SRV-{job.serviceId}</span>
                    </div>
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${getStatusBadgeColor(job.serviceStatus)}`}>
                ...
                <div className="space-y-2 mb-4 text-sm text-slate-600">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <a href={`tel:${job.contactNo}`} className="hover:underline text-blue-600">{job.contactNo || '-'}</a>
                    </div>
                    {job.address && (
                        <div className="flex items-start gap-2">
                            <MapPin size={14} className="text-slate-400 shrink-0 mt-0.5" />
                            <span className="line-clamp-2" title={job.address}>{job.address}</span>
                        </div>
                    )}
                </div>
                ...
                <div className="mb-4 pt-3 border-t border-slate-100">
                    <div className="flex justify-between text-sm mb-1.5">
                        ...
                        <span className="font-bold text-slate-800">Rs. {Number(job.serviceCharge || 0).toLocaleString()}</span>
                    </div>
                    {job.notes && (
                ...
                {job.serviceStatus === 'IN_PROGRESS' && (
                    <div className="mt-4 pt-3 border-t border-slate-100 space-y-3">
                        <div>
                            <label className="text-xs font-bold text-slate-500 mb-1 block">Balance Collected (Rs.)</label>
                            <input
                                type="number"
                                min="0"
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-500 mb-1 block">Additional Items / Materials</label>
                            <input
                                type="text"
            ...
            <div className="flex items-center gap-2 mt-5">
                ...
                <button
                    ...
                    className={`flex-1 py-2.5 rounded-xl font-bold transition-all flex items-center justify-center gap-2 shadow-sm text-sm active:scale-[0.98] ${getActionBtnColor(actionColor)} disabled:opacity-50 disabled:cursor-not-allowed`}
                >
                    {isProcessing ? <Activity size={16} className="animate-spin" /> : icon}
                    {actionText}
                </button>
            </div>
        </div>
