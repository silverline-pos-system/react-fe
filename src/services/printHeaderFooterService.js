...
export async function fetchBranchPrintHeaderFooterSettings(branch) {
  ...
  try {
    ...
    // If 403 (Forbidden - permission denied), 404 (Not found), or other 4xx status
    // silently fall back to cached/default settings without logging errors
    if (response.status === 403 || response.status === 404) {
      // User doesn't have permission or settings don't exist - use defaults
      return getBranchPrintHeaderFooterSettings(branch);
    }
    
    // For other 4xx status codes, use cached/default settings
    return getBranchPrintHeaderFooterSettings(branch);
  } catch {
    // Network error or other exception - fallback to cached or default settings
    return getBranchPrintHeaderFooterSettings(branch);
  }
}
