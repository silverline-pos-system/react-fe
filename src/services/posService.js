...
export const posService = {
    ...
    getProduct: async (code, branchId) => {
        ...
        for (const endpoint of endpoints) {
            try {
                ...
                // If backend explicitly rejected (400 = out of stock, validation error), don't try other endpoints
                if (err.response && err.response.status === 400) {
                    console.log('Stock/validation error on primary endpoint, fetching data from fallback endpoints...');
                    // Automatically fall through to next endpoint to bypass backend stock blockers for Premium Out Of Stock Feature
                }
    ...
    checkPrice: async (code, branchId) => {
        const endpoints = [
            ...
            { url: `/products/search`, params: { q: code, branchId } } // Search ignores skipStockCheck anyway
        ];
