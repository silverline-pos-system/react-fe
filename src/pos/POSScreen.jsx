...
import { getApprovals } from '../services/managerService';
import { NotificationProvider, useNotification } from './context/NotificationContext';
import NotificationPanel from './components/NotificationPanel';
import { printReceiptPDF, printShiftSummary, printPayInOutReceipt } from '../utils/receiptPrinter';
...
import ReloadModal from './modals/ReloadModal'; // Added ReloadModal
...
import DtvRequestModal from './modals/DtvRequestModal';
import MobileRepairModal from './modals/MobileRepairModal';
import PriceSelectionModal from './modals/PriceSelectionModal';
import SerialSelectionModal from './modals/SerialSelectionModal';
...
function POSContent() {
    ...
    const fetchSupplierPaymentCount = useCallback(async () => {
        try {
            ...
            // 403 = cashier role lacks permission — silently ignore
    ...
    const handleLogout = async (closingAmount, supervisorCreds, denominations = null) => {
        try {
            ...
            // Print shift summary report before redirecting
            try {
                printShiftSummary({
                    cashierName: session.cashier,
            ...
            // Clear notifications and auth data on successful close shift
    ...
    const handleAddToCart = async (productInput) => {
        ...
        try {
            ...
            // If productInput is already a product object (from quick pick or suggestions)
            if (typeof productInput === 'object' && productInput !== null) {
                rawData = productInput;
                
                // Always fetch full product details for physical products to get latest availablePrices
                // This ensures the price selection modal appears every time for multi-price products
            ...
            if (!product.isService) {
                // Check for multi-pricing batches before proceeding
                if (rawData.availablePrices && rawData.availablePrices.length > 0) {
                    // Aggregate batches by selling price (using fixed-point string key for accuracy)
            ...
            const processAdd = () => {
                ...
                let newCartLength = 0;
                setCart(prev => {
                    // For serialized items, always add as a new line item, do not merge.
                    // For non-serialized items, check for existing.
                ...
                // If serialized but no serial ID (e.g. added via SKU/Barcode), prompt for serial
                ...
                // Set timeout to allow batch state updates to clear
                setTimeout(() => {
            ...
            // Skip stock check for service items like DTV, Repairs, Reloads
            // Premium Feature: Allow selling out of stock / overriding stock limit with confirmation if enabled by admin
            if (!product.isService && stock != null && newQty > stock) {
                // If out-of-stock sales are allowed, show confirmation dialog
                if (isOosAllowed) {
                    showConfirm(
                    ...
                    // Out-of-stock sales not allowed - block the addition
                    if (stock <= 0) {
    ...
    // Handle multi-price product selection from PriceSelectionModal
    // Receives the selected selling price (number) from the modal
    const _handleAddMultiPriceProduct = (selectedPrice) => {
        ...
        // Find the matching price group from our aggregated options
        const priceGroup = options.find(g => Math.abs(g.sellingPrice - parseFloat(selectedPrice)) < 0.01);

        // Update the product with the selected price and batch info
    ...
    const handleScan = (overrideValue) => {
        ...
        if (!text) {
            // When Enter is pressed with an empty input buffer, trigger cash payment
            if (cart.length > 0) {
    ...
    const openPaymentModal = (mode = 'CASH') => {
        ...
        // Check if cart has return items and net total is still negative/zero
    ...
    const processPayment = async (paymentDetails) => {
        ...
        // Filter out service items (repairs, DTV) and return items (already processed)
        ...
        // Determine if this is a service-only sale (no product items)
        ...
        const orderData = {
            ...
            notes: isServiceOnly
                ...
                : "",
            items: mappedItems,
            payments: paymentList // Backend expects payments
        };
        ...
        try {
            ...
            // Sync DTV service requests that were added to cart
            for (const item of cart) {
                ...
                // Mark repair jobs as PAID/DELIVERED after successful sale
                if (item.repairData) {
                    try {
    ...
    const handleReloadAdd = (reloadDetails) => {
        ...
        const reloadItem = {
            ...
            qty: 1,
            isService: true,
            type: 'RELOAD', // Identify as reload
            taxRate: 0,
            discount: 0
        };
        ...
        <div className="h-screen w-screen flex flex-col bg-slate-100 overflow-hidden font-sans relative">
            ...
            <header className="bg-slate-900 text-white h-16 shrink-0 flex items-center justify-between px-4 shadow-md z-30 border-b border-slate-800">
                ...
                <div className="flex items-center gap-6">
                    ...
                    {/* Supplier Payment Request Notification */}
            ...
            {activeModal === 'SMART_RETURN' && (
                <SmartReturnModal
                    ...
                    onReturnToCart={(negativeItems, refundTotal) => {
                        // Add returned items to cart as negative-value entries
            ...
            {activeModal === 'PRICE_SELECT' && pendingMultiPriceProduct && (
                <PriceSelectionModal
                    ...
                    onSelectPrice={(selectedPrice) => {
                        // Find the selected option to get batchId and stock
