...
import { printMobileRepairReceipt } from '../../utils/receiptPrinter';
...
export default function MobileRepairModal({ onClose, branchId, onNotify, onAddToCart, enablePrintReceipt = false, branchInfo = null, cashierName = '' }) {
    ...
    const [activeTab, setActiveTab] = useState('LOG'); // 'LOG' or 'FIND'
    ...
    const formatRepairNo = (repair) => {
        ...
        // Keep backend values only if they already look like a structured ticket/invoice code.
    ...
    const handleAddRepairToCart = async (repair) => {
        ...
        // Add repair service as a cart item
    ...
    const _handleQuickPay = async (repair) => {
        const balanceDue = parseFloat(repair.balanceDue) || parseFloat(repair.finalCost) || 0;
        if (balanceDue <= 0) {
            onNotify('info', 'No Balance', 'This repair has no outstanding balance.');
            return;
        ...
        try {
            ...
            await servicesService.markRepairPaid(repair.repairId, balanceDue, 'CASH', userId);
            onNotify('success', 'Payment Recorded', `LKR ${balanceDue.toLocaleString()} paid for Repair ${formatRepairNo(repair)}. Device ready for delivery.`);
            // Refresh search results
            handleSearch();
        } catch (error) {
            console.error('Payment failed:', error);
            onNotify('error', 'Payment Failed', error.response?.data?.message || 'Failed to record payment.');
        } finally {
            _setPayingId(null);
        }
    };
